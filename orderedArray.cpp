...
void orderedArray::copyFromUnordered(string *newData, int *newNum,
                                     long arraySize) {    //Implement quicksort on a string array
    this->size = arraySize;                                                             //and keep it in relation to an int array
    this->data = new string[size];
    this->num = new int[size];
    copy(&newData[0], &newData[arraySize - 1],
         this->data);                      //Copy two arrays passed by reference locally
